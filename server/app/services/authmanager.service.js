	...
	__awaitAuthorization(dialog) {
		...
		var codePromise = new Promise((resolve, reject) => {
			try {
				...
				httpListener = http.createServer(async (req, res) => {
					try {
						...
						if (urlParams.code) {
							...
							if (this.debugMode) {
								console.debug(
									'Received a code from Google server.'
								);
							}

							// awaits the tokens using the code
							const tokens = await this.googleAPI.generateToken(
								code
							);

							if (this.debugMode) {
								console.debug(
									'Received tokens from Google server.'
								);
							}
							...
							if (this.debugMode) {
								console.debug(
									'Applied tokens to the authenticator.'
								);
							}
						...
						console.error(
							'Error in authorizing with Google.',
							error
						);
						reject(error);
					}
				...
				if (this.debugMode) {
					console.debug(
						'Listening for authorization confirmation from Google.'
					);
				}
				...
				setTimeout(() => {
					try {
						...
						dialog.close();
						console.error(
							'Could not get any response from Google.'
						);
